...
import sv_ttk
from popup import Pop_Window
from filterPop import Filter_Window
...
def check_conn():                               # Check for connectivity with existing MySQL credentials
    try:
        init_f = open("connect.cnf", "r")
        init_conData = init_f.readlines()
        init_us = init_conData[1].replace("user = ", "").strip()
        init_pw = init_conData[2].replace("password = ", "").strip()
        init_hs = init_conData[3].replace("host = ", "").strip()
        return db.ping(init_hs, init_us, init_pw)
